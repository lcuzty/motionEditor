...
class URDFLoader {
    ...
    parse(content, workingPath = this.workingPath) {
        ...
        function resolvePath(path) {
            console.log(`[URDFLoader.resolvePath] 输入路径: ${path}`)

            if (!/^package:\/\//.test(path)) {
                // 如果workingPath是blob URL，不要拼接，直接返回相对路径
                // 让urlModifier来处理路径解析
                if (workingPath && workingPath.startsWith('blob:')) {
                    console.log(`[URDFLoader.resolvePath] workingPath是blob URL，直接返回相对路径: ${path}`)
                    return path;
                }
                const result = workingPath ? workingPath + path : path;
                console.log(`[URDFLoader.resolvePath] 非package路径，返回: ${result}`)
                return result;
            ...
            console.log(`[URDFLoader.resolvePath] package路径 - targetPkg: ${targetPkg}, relPath: ${relPath}`)
            ...
            if (typeof packages === 'string') {
                ...
                if (packages.endsWith(targetPkg)) {
                    ...
                    const result = packages + '/' + relPath;
                    console.log(`[URDFLoader.resolvePath] 匹配到package(string)，返回: ${result}`)
                    return result;

                } else {

                    // Assume "pkg" is the target package's parent directory
                    const result = packages + '/' + targetPkg + '/' + relPath;
                    console.log(`[URDFLoader.resolvePath] 假定为父目录，返回: ${result}`)
                    return result;
                ...
                const result = packages(targetPkg) + '/' + relPath;
                console.log(`[URDFLoader.resolvePath] 使用function解析，返回: ${result}`)
                return result;
                ...
                if (targetPkg in packages) {
                    ...
                    const result = packages[targetPkg] + '/' + relPath;
                    console.log(`[URDFLoader.resolvePath] 匹配到package(object)，返回: ${result}`)
                    return result;
    ...
    defaultMeshLoader(path, manager, done) {
        console.log(`[URDFLoader.defaultMeshLoader] 准备加载mesh: ${path}`)

        if (/\.stl$/i.test(path)) {
            console.log(`[URDFLoader.defaultMeshLoader] 识别为STL文件，使用STLLoader加载`)
            const loader = new STLLoader(manager);
            loader.load(path, geom => {
                console.log(`[URDFLoader.defaultMeshLoader] ✅ STL文件加载成功: ${path}`)
                const mesh = new THREE.Mesh(geom, new THREE.MeshPhongMaterial());
                done(mesh);
            }, undefined, (error) => {
                console.error(`[URDFLoader.defaultMeshLoader] ❌ STL文件加载失败: ${path}`, error)
                done(null, error);
            });

        } else if (/\.dae$/i.test(path)) {
            console.log(`[URDFLoader.defaultMeshLoader] 识别为DAE文件，使用ColladaLoader加载`)
            const loader = new ColladaLoader(manager);
            loader.load(path, dae => {
                console.log(`[URDFLoader.defaultMeshLoader] ✅ DAE文件加载成功: ${path}`)
                done(dae.scene)
            }, undefined, (error) => {
                console.error(`[URDFLoader.defaultMeshLoader] ❌ DAE文件加载失败: ${path}`, error)
                done(null, error);
            });
